-- First check what foreign key constraints exist and remove them if needed
-- Then insert test data and enable real-time

-- Drop foreign key constraints that are preventing inserts
DO $$
BEGIN
    -- Remove foreign key constraint from video_views if it exists
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
               WHERE constraint_name = 'video_views_user_id_fkey' 
               AND table_name = 'video_views') THEN
        ALTER TABLE video_views DROP CONSTRAINT video_views_user_id_fkey;
    END IF;
    
    -- Remove foreign key constraint from compartilhamentos if it exists
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
               WHERE constraint_name = 'compartilhamentos_usuario_id_fkey' 
               AND table_name = 'compartilhamentos') THEN
        ALTER TABLE compartilhamentos DROP CONSTRAINT compartilhamentos_usuario_id_fkey;
    END IF;
    
    -- Remove foreign key constraint from user_sessions if it exists  
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
