...
-- Create user profiles table for authentication (if not exists)
...
CREATE POLICY "Users can view their own profile" ON public.users
    FOR SELECT USING (auth.uid()::text = id OR auth.uid()::text IN (
...
DROP FUNCTION IF EXISTS public.testar_sistema();

-- Update existing functions with proper security settings
CREATE OR REPLACE FUNCTION public.get_daily_points_status(p_user_id uuid, p_action_type character varying)
RETURNS json
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
    ...
    result JSON;
BEGIN
    -- Verify the calling user can access this data
    ...
    -- Se não existe registro, é primeira vez
    IF NOT FOUND THEN
    ...
    ELSE
        ...
        hours_diff := EXTRACT(EPOCH FROM (NOW() - action_record.created_at)) / 3600;
