...
-- 5. Fix user_sessions table - Users can only see their own sessions (CORRECTED COLUMN NAME)
...
-- Create secure policies for user_sessions using correct column name
...
-- 6. Additional security: Remove duplicate bonus_users policy
DROP POLICY IF EXISTS "Allow registration of new bonus users" ON public.bonus_users;

-- 7. Allow limited public access for basic app functionality
...
INSERT INTO public.audit_logs (
  ...
  'SECURITY_FIX',
  'multiple_tables', 
  '{"description": "Fixed critical RLS policies preventing data theft", "tables_fixed": ["email_logs", "integrations", "profiles", "system_settings", "user_sessions"], "severity": "critical", "threat_prevented": "Personal information theft by hackers"}'::jsonb,
